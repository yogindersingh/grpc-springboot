...
import com.learning.domain.PriceUpdate;
import com.learning.tests.mockservice.StockMockService;
...
public class StockUpdatesTest {
    ...
    private static final String STOCK_UPDATES_ENDPOINT = "http://localhost:%d/stockUpdates";
    ...
    public void stockUpdatesTest(){
        List<PriceUpdate> list = this.restTemplate.execute(
        ...
        Assertions.assertEquals(10, list.get(0).price());
    }

    private List<PriceUpdate> getResponse(ClientHttpResponse clientHttpResponse){
        var list = new ArrayList<PriceUpdate>();
        try(var reader = new BufferedReader(new InputStreamReader(clientHttpResponse.getBody()))){
            String line;
            while(Objects.nonNull(line = reader.readLine())){
                if(!line.isEmpty()){
                    log.info(line);
                    var dto = mapper.readValue(line.substring("data:".length()), PriceUpdate.class);
                    list.add(dto);
                }
            }
        }catch (Exception e){
